...
var _applyFormatingRules = function (value, rule) {
...
var _getPageUrl = function (spec, title, chapter, page) {

  // # Manage rules on chapter
  var chapterCode = _applyFormatingRules(chapter, spec.chapterRule);

  // # Manage rules on page
  var pageCode = _applyFormatingRules(page, spec.pageRule);
  ...
  var pageUrl = _tpl(spec.pageURL, values);
...
export default {
  ...
  applyFormatingRules: function (value, rule) {
    "use strict";
    return _applyFormatingRules(value, rule);
  },
  ...
  getPageUrl: function (spec, title, chapter, page) {
    ...
    return _getPageUrl(spec, title, chapter, page);
    ...
    Object.keys(volumes).forEach(volume => {
      ...
      for (var chapter = chapters.begin; chapter <= chapters.end; chapter++) {
        ...
        for (var page = PAGE_BEGIN; page < PAGE_END; page++) {
          ...
          var pageUrl = _getPageUrl(specification, title, chapter, page);

          // #### Compute the name of the image local file
          var chapterString = _applyFormatingRules(chapter, 'fixed3');
          var pageString = _applyFormatingRules(page, 'fixed3');
  ...
  createCbz: function (volume, pageList) {
    ...
    var output = fs.createWriteStream('out/' + volume + '.cbz');
    ...
    archive.on('error', function (err) {
      console.log('Zip error on volume: out/' + volume + '.cbz - ' + err);
    });
